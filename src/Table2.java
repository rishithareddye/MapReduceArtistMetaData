...
public class Table2
{
   public static class Map extends Mapper<LongWritable, Text, Text, IntWritable>
   {
            ...
            throws IOException, InterruptedException
      {
         ...
         final String[] words = line.split("<SEP>");//splitting the value using seperator 
         final int size = words.length;
         if (size == 4)// checking the length
         {
            final String location = words[3].trim();//storing the location

            con.write(new Text(location), new IntWritable(1));//sending <key,1> to reducer(wordcount logic)
         }
        }
   }
   ...
   public static class Red extends Reducer<Text, IntWritable, Text, IntWritable>
   {
      ...
      public void reduce(final Text key, final Iterable<IntWritable> Values, final Context con)
            throws IOException, InterruptedException
      {
         final String location = key.toString().trim();// storing location(key)
         int sum = 0;
         for (final IntWritable val : Values)// for every unique location sum=sum+1
         {
            sum += val.get();//sum=sum+1
         }
         con.write(new Text(location.trim() + "<SEP>"+sum), null);//writing key<SEP>sum(number of repetition)
      }
   }
   ...
   public static void main(final String[] args) throws Exception
   {
      ...
      {
    	  copyFile(new File(args[0]+"/part-r-00000"),new File("input/Table2.txt"));// copy file part-r-00000 to Table2.txt for future use
    	  System.exit(0);
      }
      else
    	  System.exit(1);
